...
#include <core/external_2D.h>
#include <matrix/matrix_expr.h>
#include <matrix/class_ops.h>
#include <matrix/matrix_unroller.h>
...
: public external_2D<Element,Rows,Cols,Layout>
{
  public:
    ...
    typedef IEMath::et::matrix_result_tag result_tag;
    ...
    typedef matrix<
        typename IEMath::remove_const<Element>::type,
...
: public external_2D<Element,-1,-1,Layout>
{
  ...
  public:
    ...
    void random(ELEMENT_ARG_TYPE min, ELEMENT_ARG_TYPE max) {
      for(size_t i = 0; i < this->rows(); ++i) {
        for(size_t j = 0; j < this->cols(); ++j) {
          (*this)(i,j) = IEMath::random_real(min,max);
        }
      }
    }
