...
struct inverse_f
{
    typename MatT::temporary_type operator()(const MatT& M) const
    {
        ...
        IEMath::et::detail::Resize(LU,N,N);
        LU = lu(M);
        ...
        IEMath::et::detail::Resize(v,N);
        IEMath::et::detail::Resize(x,N);
        for(size_t i = 0; i < N; ++i)
            v[i] = value_type(0);
...
inverse(const MatT& M, fixed_size_tag/*, bool force_NxN*/)
{
    ...
    IEMath::et::CheckedSquare(M, fixed_size_tag());
...
{
    /* Require a square matrix: */
    IEMath::et::CheckedSquare(M, dynamic_size_tag());
