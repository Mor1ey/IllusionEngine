...
void lu_inplace(MatT& A)
{
    ...
    ssize_t N = (ssize_t) IEMath::et::CheckedSquare(A, size_tag());
...
lu_copy(const MatT& M)
{
    ...
    IEMath::et::detail::Resize(A,M.rows(),M.cols());
    A = M;
    lu_inplace(A);
    return A;
}
...
lu_solve(const MatT& LU, const VecT& b)
{
    ...
    /* Verify that the matrix is square, and get the size: */
    ssize_t N = (ssize_t) IEMath::et::CheckedSquare(
     ...
     * 1's:
     */
    vector_type y; IEMath::et::detail::Resize(y,N);
     ...
     */
    vector_type x; IEMath::et::detail::Resize(x,N);
