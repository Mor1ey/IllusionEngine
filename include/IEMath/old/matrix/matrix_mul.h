          ...
          const et::MatrixXpr<XprT>& right)
{
    ...
    expr_tmp tmp;
    IEMath::et::detail::Resize(tmp,right.rows(),right.cols());
    tmp = right;
          ...
          const matrix<E,AT,BO,L>& right)
{
    ...
    typedef typename et::MatrixXpr<XprT>::temporary_type expr_tmp;
    expr_tmp tmp;
    IEMath::et::detail::Resize(tmp,left.rows(),left.cols());
    tmp = left;
          ...
          const et::MatrixXpr<XprT2>& right)
{
    ...
    left_tmp ltmp;
    IEMath::et::detail::Resize(ltmp,left.rows(),left.cols());
    ltmp = left;

    typedef typename et::MatrixXpr<XprT2>::temporary_type right_tmp;
    right_tmp rtmp;
    IEMath::et::detail::Resize(rtmp,right.rows(),right.cols());
    rtmp = right;
