...
#include <core/external_1D.h>
#include <vector/vector_expr.h>
#include <vector/class_ops.h>
#include <vector/vector_unroller.h>
...
: public external_1D<Element,Size>
{
  public:
    ...
    /* For integration into the expression template code: */
    typedef vector<typename IEMath::remove_const<Element>::type,
    ...
    typedef IEMath::et::vector_result_tag result_tag;

    /* For matching by assignability: */
    typedef IEMath::et::assignable_tag assignable_tag;
  ...
  public:
    ...
    vector_type& zero() {
        typedef IEMath::et::OpAssign<Element,Element> OpT;
        IEMath::et::UnrollAssignment<OpT>(*this,Element(0));
        return *this;
    }
    ...
    void random(value_type min, value_type max) {
        for (size_t i = 0; i < this->size(); ++i) {
            (*this)[i] = IEMath::random_real(min,max);
        }
    }
  ...
  public:
    ...
    CML_VEC_ASSIGN_FROM_VEC(=, IEMath::et::OpAssign)
    CML_VEC_ASSIGN_FROM_VEC(+=, IEMath::et::OpAddAssign)
    CML_VEC_ASSIGN_FROM_VEC(-=, IEMath::et::OpSubAssign)

    CML_VEC_ASSIGN_FROM_VECXPR(=, IEMath::et::OpAssign)
    CML_VEC_ASSIGN_FROM_VECXPR(+=, IEMath::et::OpAddAssign)
    CML_VEC_ASSIGN_FROM_VECXPR(-=, IEMath::et::OpSubAssign)

    CML_VEC_ASSIGN_FROM_SCALAR(*=, IEMath::et::OpMulAssign)
    CML_VEC_ASSIGN_FROM_SCALAR(/=, IEMath::et::OpDivAssign)
};
